...
sof      = json.loads(pathlib.Path("test_sof_mazatlan.json").read_text())
invoice  = json.loads(pathlib.Path("test_invoice_mazatlan.json").read_text())
tariff   = json.loads(pathlib.Path("tariffs/mazatlan.json").read_text())
...
print()
print("=== ADJUSTMENT LINES ===")
for a in adjustment_lines:
    sign = "+" if a["amount"] >= 0 else ""
    print(f"  {a['description']}: USD {sign}{a['amount']:,.2f}")
    print(f"    note: {a['note']}")
print()
print("=== NOTES ===")
